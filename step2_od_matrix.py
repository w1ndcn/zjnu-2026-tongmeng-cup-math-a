...
====================
基于引力模型 + 距离阶梯方式划分，
生成分时段（早/午/晚）× 分方式（步行/电动车/机动车）的OD需求矩阵。
"""
...
ATTENDANCE = 0.45  # 早高峰上课率：宿舍出发的学生中仅45%实际进入教学楼
...
PERIOD_FLOW = {
    ...
    'noon': {
        ...
        'overflow': ['TYGY', 'XYGY', 'CYGY', 'GYGY'],  # 食堂满了→回宿舍
    },
    'evening': {
        'sources': ['T_16', 'T_25', 'T_1', 'T_2', 'T_3', 'T_4', 'T_5',
                     'T_6', 'T_7', 'T_8', 'T_9', 'T_10'],
...
def main():
    ...
    for period in PERIODS:
        ...
        if period == 'morning':
            ...
            # 午高峰：先引力分配到食堂/商业街→容量约束→溢出到宿舍按比例分
            od_total = gravity_model(sources, targets, dist_matrix, node_idx, lam, node_order)
